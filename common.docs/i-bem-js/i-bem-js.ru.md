...
Использовать в [декларации](#decl) имя блока в качестве первого аргумента `modules.define`.
...
Включить класс `i-bem` в список классов HTML-элемента. Наличие этого класса укажет фреймворку, что нужно инициализировать блок.
...
Чтобы привязать экземпляр блока к нескольким HTML-элементам, в атрибуте `data-bem` всех привязываемых HTML-элементов нужно указать одинаковое значение `id`.
...
экземпляра блока. Значение `id` должно быть уникальным для
 ...
 * декларируются как [доопределение модуля `i-bem`](#bem-decl);
...
Доступ к параметрам из экземпляра блока можно получить через поле `this.params`. Его значение – хеш параметров из атрибута `data-bem` DOM-элемента блока (`this.domElem`).
...
```js
modules.define('my-block', ['i-bem_dom'], function(provide, BEMDOM) {
...
Для получения **параметров элемента** служит метод экземпляра блока `elemParams`. Он принимает аргументом строку с именем элемента или его jQuery-объект. Возвращает хеш параметров элемента.

```html
<div class="my-block i-bem" data-bem='{ "my-block": {} }'>
    <div class="my-block__my-elem i-bem" data-bem='{ "my-block__my-elem": { "foo" : "bar" } }'></div>
</div>
```
...
Чтобы задекларировать новый JS-блок **с DOM-представлением** (привязанный к HTML-элементу), нужно воспользоваться методом `decl` [ymaps][]-модуля `i-bem__dom`.
...
```js
modules.define('button', ['i-bem_dom'], function(provide, BEMDOM) {
...
* имя (и значение) модификатора. Объявленные методы будут применяться только к тем
экземплярам блока, у которых присутствует данный модификатор (с указанным значением);
* список родительских блоков, методы которых блок должен наследовать.
...
Для декларации блоков без DOM-представления служит метод `decl` [ymaps][]-модуля `i-bem`.
...
Для доступа к методам родителя служит [специальное поле декларации](#decl-context) `this.__base`.
...
* хеш методов, которые будут доступны для блока с соответствующим модификатором. Пересекающиеся методы будут переопределены методами из хеша.
...
Чтобы подмешать к блоку один или несколько блоков-миксов, необходимо в декларации блока присвоить значение опциональному полю `baseMix`. Значением служит массив строк – имен примешиваемых блоков-миксов:
...
Различие в том, что первым аргумент `declMix` – хеш вида `{ block : 'blockname' }`. Без указания модификаторов или примешиваемых блоков:
...
* `blocks` – хранилище деклараций блоков. Содержит хеш, ключами которого являются имена блоков. Служит для получения доступа к классам блоков.
...
или отсутствие модификатора у блока, а его значение несущественно. Например, модификатор описывающий состояние «отключен»:
...
Экземпляр блока предоставляет методы для установки, удаления и проверки значений модификаторов данного экземпляра.

**NB** Для изменения значений модификаторов используйте описанный ниже API, предоставляемый экземпляром блока. Не следует устанавливать модификаторы, самостоятельно изменяя CSS-классы соответствующего DOM-узла.
...
* `getMods([elem], [...modNames])` – возвращает хеш со значениями всех модификаторов блока и его элементов. Можно ограничить действие метода указав нужный элемент `[elem]` или `[...modNames]` – строку с именами модификаторов разделенными пробелом.
...
В качестве параметра функции-обработчику передается объект, описывающий БЭМ-событие. Класс объекта БЭМ-события `events.Event` определен в [ymaps][]-модуле [`events`](https://github.com/bem/bem-core/blob/v2/common.blocks/events/events.vanilla.js) библиотеки bem-core. Содержит поля:
...
[ymaps-модуля][ymaps] (без использования `i-bem`) нет необходимости создавать экземпляр блока.
